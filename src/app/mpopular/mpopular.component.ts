...
@Component({
  selector: 'app-mpopular',
  templateUrl: './mpopular.component.html',
  styleUrls: ['./mpopular.component.css']
})
export class MpopularComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.myMS.getpopularMovies(this.currentPage).subscribe({
      next: (response) => {
  ...
  searchMovies(movieName: string) {
    this.myMS.searchpopularMovies(movieName, this.currentPage).subscribe({
      next: (data) => {
  ...
  changeLanguage() {
    ...
    this.myMS.getpopularMovies().subscribe({
      next: (response) => {
        this.allMovies = response.results;
      },
    });
